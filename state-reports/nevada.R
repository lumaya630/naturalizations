...
##### USCIS DATA #####
# Read in USCIS data
nevada <- temp %>% select("Region.Name", "country_birth", "NEVADA") %>% 
...
acs_NV20 <- data %>% subset(YEAR == 2020 & STATEFIP == 32)
acs_NV20_imm <- acs_NV20 %>% subset(BPL > 120)
acs_NV20_natz <- acs_NV20 %>% subset(CITIZEN == 2) 
acs_NV20_newNatz <- acs_NV20_natz %>% subset(YRNATUR >=2016)

###### ANALYSIS ###### 
#============================================================================
# NEVADA POPULATION COUNTS
#============================================================================
###### CALCULATE NUMBER OF IMMIGRANTS, TOTAL NATURALIZED IN NEVADA ######
sum(acs_NV20$PERWT) # nevada population
sum(acs_NV20_imm$PERWT) # number of immigrants in nevada
sum(acs_NV20_natz$PERWT) # number of naturalized citizens in nevada
sum(acs_NV20_newNatz$PERWT)  # number of newly naturalized citizens in nevada

####### CALCULATE NUMBER ELIGIBLE FOR CITIZENSHIP ####################
eligible_citizen <- acs_NV20_imm %>% subset(CITIZEN %in% c(3, 4) & YRSUSA1 > 5)
sum(eligible_citizen$PERWT)

####### PRE-TAX WAGE CONTRIBUTIONS ####################
# get rid of 99999/99998 values representing N/A or Missing incomes
incwage <- (acs_NV20_imm %>% subset(INCWAGE < 999998))
...
n_newnatz = 42976
natz_by_region <- as.data.frame(nevada %>% group_by(Region.Name) %>% summarise(Freq = sum(NEVADA)) %>%
                                  mutate(Percentage = Freq/n_newnatz))
View(natz_by_region)

# top ten country of origins
view(nevada %>% arrange(desc(NEVADA)))
...
#============================================================================
####### CALCULATE OCCUPATION PREVALENCE OF IMMIGRANTS IN NEVADA ###########
#============================================================================
# workforce populaiton in nevada
n_nevada = sum(acs_NV20$PERWT[acs_NV20$OCC != 0])
occ_general <- (
  acs_NV20 %>% group_by(category)  %>%
    summarise(n = sum(PERWT),
              Freq = sum(PERWT)/n_nevada)
)

# immigrant workforce population
n_imm = sum(acs_NV20_imm$PERWT[acs_NV20_imm$OCC != 0])
occ_imm <- (
  acs_NV20_imm %>% group_by(category)  %>%
    summarise(n = sum(PERWT),
...
# =========================================
# export csvs
write.csv(nevada, "Country_Region_Birth_OHIO.csv", row.names = F)
write.csv(county_region, "Natz_by_County_OHIO.csv", row.names = F)
write.csv(natz_by_region, "Natz_by_Region_OHIO.csv", row.names = F)
write.csv(acs_NV20, "out/acs_NV20.csv", row.names = F)
write.csv(acs_NV20_imm, "out/acs_NV20_imm.csv", row.names = F)
write.csv(acs_NV20_natz, "out/acs_NV20_natz.csv", row.names = F)
write.csv(acs_NV20_newNatz, "out/acs_NV20_newNatz.csv", row.names = F)
