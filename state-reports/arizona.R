...
##### USCIS DATA #####
# Read in USCIS data
arizona <- temp %>% select("Region.Name", "country_birth", "ARIZONA") %>% 
...
acs_AZ20 <- data %>% subset(YEAR == 2020 & STATEFIP == 04)
acs_AZ20_imm <- acs_AZ20 %>% subset(BPL > 120)
acs_AZ20_natz <- acs_AZ20 %>% subset(CITIZEN == 2) 
acs_AZ20_newNatz <- acs_AZ20_natz %>% subset(YRNATUR >=2016)

###### ANALYSIS ###### 
#============================================================================
# ARIZONA POPULATION COUNTS
#============================================================================
###### CALCULATE NUMBER OF IMMIGRANTS, TOTAL NATURALIZED IN ARIZONA ######
sum(acs_AZ20$PERWT) # arizona population
sum(acs_AZ20_imm$PERWT) # number of immigrants in arizona
sum(acs_AZ20_natz$PERWT) # number of naturalized citizens in arizona
sum(acs_AZ20_newNatz$PERWT)  # number of newly naturalized citizens in arizona

####### CALCULATE NUMBER ELIGIBLE FOR CITIZENSHIP ####################
eligible_citizen <- acs_AZ20_imm %>% subset(CITIZEN %in% c(3, 4) & YRSUSA1 > 5)
sum(eligible_citizen$PERWT)

####### PRE-TAX WAGE CONTRIBUTIONS ####################
# get rid of 99999/99998 values representing N/A or Missing incomes
incwage <- (acs_AZ20_imm %>% subset(INCWAGE < 999998))
...
n_newnatz = 63857
natz_by_region <- as.data.frame(arizona %>% group_by(Region.Name) %>% summarise(Freq = sum(ARIZONA)) %>%
                                  mutate(Percentage = Freq/n_newnatz))
View(natz_by_region)

# top ten country of origins
view(arizona %>% arrange(desc(ARIZONA)))
...
#============================================================================
####### CALCULATE OCCUPATION PREVALENCE OF IMMIGRANTS IN ARIZONA ###########
#============================================================================
# workforce populaiton in arizona
n_arizona = sum(acs_AZ20$PERWT[acs_AZ20$OCC != 0])
occ_general <- (
  acs_AZ20 %>% group_by(category)  %>%
    summarise(n = sum(PERWT),
              Freq = sum(PERWT)/n_arizona)
)

# immigrant workforce population
n_imm = sum(acs_AZ20_imm$PERWT[acs_AZ20_imm$OCC != 0])
occ_imm <- (
  acs_AZ20_imm %>% group_by(category)  %>%
    summarise(n = sum(PERWT),
...
# =========================================
# export csvs
write.csv(arizona, "Country_Region_Birth_OHIO.csv", row.names = F)
write.csv(county_region, "Natz_by_County_OHIO.csv", row.names = F)
write.csv(natz_by_region, "Natz_by_Region_OHIO.csv", row.names = F)
write.csv(acs_AZ20, "out/acs_AZ20.csv", row.names = F)
write.csv(acs_AZ20_imm, "out/acs_AZ20_imm.csv", row.names = F)
write.csv(acs_AZ20_natz, "out/acs_AZ20_natz.csv", row.names = F)
write.csv(acs_AZ20_newNatz, "out/acs_AZ20_newNatz.csv", row.names = F)
