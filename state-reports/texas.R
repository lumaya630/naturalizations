...
incwage <- (acs_TX20_imm %>% subset(INCWAGE < 999998))
...
#============================================================================
####### CALCULATE OCCUPATION PREVALENCE OF IMMIGRANTS IN TEXAS ###########
#============================================================================
# workforce populaiton in texas
n_texas = sum(acs_TX20$PERWT[acs_TX20$OCC != 0])
occ_general <- (
  acs_TX20 %>% group_by(category)  %>%
    summarise(n = sum(PERWT),
              Freq = sum(PERWT)/n_texas)
)

# immigrant workforce population
n_imm = sum(acs_TX20_imm$PERWT[acs_TX20_imm$OCC != 0])
occ_imm <- (
  acs_TX20_imm %>% group_by(category)  %>%
    summarise(n = sum(PERWT),
...
# =========================================
# export csvs
write.csv(texas, "Country_Region_Birth_OHIO.csv", row.names = F)
write.csv(county_region, "Natz_by_County_OHIO.csv", row.names = F)
write.csv(natz_by_region, "Natz_by_Region_OHIO.csv", row.names = F)
write.csv(acs_TX20, "out/acs_TX20.csv", row.names = F)
write.csv(acs_TX20_imm, "out/acs_TX20_imm.csv", row.names = F)
write.csv(acs_TX20_natz, "out/acs_TX20_natz.csv", row.names = F)
write.csv(acs_TX20_newNatz, "out/acs_TX20_newNatz.csv", row.names = F)
