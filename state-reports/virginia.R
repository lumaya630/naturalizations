...
#============================================================================
####### CALCULATE OCCUPATION PREVALENCE OF IMMIGRANTS IN VIRGINIA ###########
#============================================================================
# workforce populaiton in virginia
n_virginia = sum(acs_VA20$PERWT[acs_VA20$OCC != 0])
occ_general <- (
  acs_VA20 %>% group_by(category)  %>%
    summarise(n = sum(PERWT),
              Freq = sum(PERWT)/n_virginia)
)

# immigrant workforce population
n_imm = sum(acs_VA20_imm$PERWT[acs_VA20_imm$OCC != 0])
occ_imm <- (
  acs_VA20_imm %>% group_by(category)  %>%
    summarise(n = sum(PERWT),
...
# =========================================
# export csvs
write.csv(virginia, "Country_Region_Birth_OHIO.csv", row.names = F)
write.csv(county_region, "Natz_by_County_OHIO.csv", row.names = F)
write.csv(natz_by_region, "Natz_by_Region_OHIO.csv", row.names = F)
write.csv(acs_VA20, "out/acs_VA20.csv", row.names = F)
write.csv(acs_VA20_imm, "out/acs_VA20_imm.csv", row.names = F)
write.csv(acs_VA20_natz, "out/acs_VA20_natz.csv", row.names = F)
write.csv(acs_VA20_newNatz, "out/acs_VA20_newNatz.csv", row.names = F)
