...
##### USCIS DATA #####
# Read in USCIS data
florida <- temp %>% select("Region.Name", "country_birth", "FLORIDA") %>% 
  subset(!is.na(Region.Name))

###### ACS DATA ######
# load acs data
if (!require("ipumsr")) stop("Reading IPUMS data into R requires the ipumsr FLckage. It can be installed using the following command: install.FLckages('ipumsr')")
...
acs_FL20 <- data %>% subset(YEAR == 2020 & STATEFIP == 12)
acs_FL20_imm <- acs_FL20 %>% subset(BPL > 120)
acs_FL20_natz <- acs_FL20 %>% subset(CITIZEN == 2) 
acs_FL20_newNatz <- acs_FL20_natz %>% subset(YRNATUR >=2016)

###### ANALYSIS ###### 
#============================================================================
# FLORIDA POPULATION COUNTS
#============================================================================
###### CALCULATE NUMBER OF IMMIGRANTS, TOTAL NATURALIZED IN FLORIDA ######
sum(acs_FL20$PERWT) # florida population
sum(acs_FL20_imm$PERWT) # number of immigrants in florida
sum(acs_FL20_natz$PERWT) # number of naturalized citizens in florida
sum(acs_FL20_newNatz$PERWT)  # number of newly naturalized citizens in florida

####### CALCULATE NUMBER ELIGIBLE FOR CITIZENSHIP ####################
eligible_citizen <- acs_FL20_imm %>% subset(CITIZEN %in% c(3, 4) & YRSUSA1 > 5)
sum(eligible_citizen$PERWT)

####### PRE-TAX WAGE CONTRIBUTIONS ####################
# get rid of 99999/99998 values representing N/A or Missing incomes
incwage <- (acs_FL20_imm %>% subset(INCWAGE < 999998))
...
n_newnatz = 429017
natz_by_region <- as.data.frame(florida %>% group_by(Region.Name) %>% summarise(Freq = sum(FLORIDA)) %>%
...
#============================================================================
####### CALCULATE OCCUPATION PREVALENCE OF IMMIGRANTS IN FLORIDA ###########
#============================================================================
# workforce populaiton in florida
n_florida = sum(acs_FL20$PERWT[acs_FL20$OCC != 0])
occ_general <- (
  acs_FL20 %>% group_by(category)  %>%
    summarise(n = sum(PERWT),
              Freq = sum(PERWT)/n_florida)
)

# immigrant workforce population
n_imm = sum(acs_FL20_imm$PERWT[acs_FL20_imm$OCC != 0])
occ_imm <- (
  acs_FL20_imm %>% group_by(category)  %>%
    summarise(n = sum(PERWT),
...
# =========================================
# export csvs
write.csv(florida, "Country_Region_Birth_OHIO.csv", row.names = F)
write.csv(county_region, "Natz_by_County_OHIO.csv", row.names = F)
write.csv(natz_by_region, "Natz_by_Region_OHIO.csv", row.names = F)
write.csv(acs_FL20, "out/acs_FL20.csv", row.names = F)
write.csv(acs_FL20_imm, "out/acs_FL20_imm.csv", row.names = F)
write.csv(acs_FL20_natz, "out/acs_FL20_natz.csv", row.names = F)
write.csv(acs_FL20_newNatz, "out/acs_FL20_newNatz.csv", row.names = F)
