...
##### USCIS DATA #####
# Read in USCIS data
georgia <- temp %>% select("Region.Name", "country_birth", "GEORGIA") %>% 
...
acs_GA20 <- data %>% subset(YEAR == 2020 & STATEFIP == 13)
acs_GA20_imm <- acs_GA20 %>% subset(BPL > 120)
acs_GA20_natz <- acs_GA20 %>% subset(CITIZEN == 2) 
acs_GA20_newNatz <- acs_GA20_natz %>% subset(YRNATUR >=2016)

###### ANALYSIS ###### 
#============================================================================
# GEORGIA POPULATION COUNTS
#============================================================================
###### CALCULATE NUMBER OF IMMIGRANTS, TOTAL NATURALIZED IN GEORGIA ######
sum(acs_GA20$PERWT) # georgia population
sum(acs_GA20_imm$PERWT) # number of immigrants in georgia
sum(acs_GA20_natz$PERWT) # number of naturalized citizens in georgia
sum(acs_GA20_newNatz$PERWT)  # number of newly naturalized citizens in georgia

####### CALCULATE NUMBER ELIGIBLE FOR CITIZENSHIP ####################
eligible_citizen <- acs_GA20_imm %>% subset(CITIZEN %in% c(3, 4) & YRSUSA1 > 5)
sum(eligible_citizen$PERWT)

####### PRE-TAX WAGE CONTRIBUTIONS ####################
# get rid of 99999/99998 values representing N/A or Missing incomes
incwage <- (acs_GA20_imm %>% subset(INCWAGE < 999998))
...
n_newnatz = 96469
natz_by_region <- as.data.frame(georgia %>% group_by(Region.Name) %>% summarise(Freq = sum(GEORGIA)) %>%
                                  mutate(Percentage = Freq/n_newnatz))
View(natz_by_region)

# top ten country of origins
view(georgia %>% arrange(desc(GEORGIA)))
...
#============================================================================
####### CALCULATE OCCUPATION PREVALENCE OF IMMIGRANTS IN GEORGIA ###########
#============================================================================
# workforce populaiton in georgia
n_georgia = sum(acs_GA20$PERWT[acs_GA20$OCC != 0])
occ_general <- (
  acs_GA20 %>% group_by(category)  %>%
    summarise(n = sum(PERWT),
              Freq = sum(PERWT)/n_georgia)
)

# immigrant workforce population
n_imm = sum(acs_GA20_imm$PERWT[acs_GA20_imm$OCC != 0])
occ_imm <- (
  acs_GA20_imm %>% group_by(category)  %>%
    summarise(n = sum(PERWT),
...
# =========================================
# export csvs
write.csv(georgia, "Country_Region_Birth_OHIO.csv", row.names = F)
write.csv(county_region, "Natz_by_County_OHIO.csv", row.names = F)
write.csv(natz_by_region, "Natz_by_Region_OHIO.csv", row.names = F)
write.csv(acs_GA20, "out/acs_GA20.csv", row.names = F)
write.csv(acs_GA20_imm, "out/acs_GA20_imm.csv", row.names = F)
write.csv(acs_GA20_natz, "out/acs_GA20_natz.csv", row.names = F)
write.csv(acs_GA20_newNatz, "out/acs_GA20_newNatz.csv", row.names = F)
