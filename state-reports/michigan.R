...
#============================================================================
####### CALCULATE OCCUPATION PREVALENCE OF IMMIGRANTS IN MICHIGAN ###########
#============================================================================
# workforce populaiton in michigan
n_michigan = sum(acs_MI20$PERWT[acs_MI20$OCC != 0])
occ_general <- (
  acs_MI20 %>% group_by(category)  %>%
    summarise(n = sum(PERWT),
              Freq = sum(PERWT)/n_michigan)
)

# immigrant workforce population
n_imm = sum(acs_MI20_imm$PERWT[acs_MI20_imm$OCC != 0])
occ_imm <- (
  acs_MI20_imm %>% group_by(category)  %>%
    summarise(n = sum(PERWT),
...
# =========================================
# export csvs
write.csv(michigan, "Country_Region_Birth_OHIO.csv", row.names = F)
write.csv(county_region, "Natz_by_County_OHIO.csv", row.names = F)
write.csv(natz_by_region, "Natz_by_Region_OHIO.csv", row.names = F)
write.csv(acs_MI20, "out/acs_MI20.csv", row.names = F)
write.csv(acs_MI20_imm, "out/acs_MI20_imm.csv", row.names = F)
write.csv(acs_MI20_natz, "out/acs_MI20_natz.csv", row.names = F)
write.csv(acs_MI20_newNatz, "out/acs_MI20_newNatz.csv", row.names = F)
