...
##### USCIS DATA #####
# Read in USCIS data
penn <- temp %>% select("Region.Name", "country_birth", "PENNSYLVANIA") %>% 
...
acs_PA20 <- data %>% subset(YEAR == 2020 & STATEFIP == 42)
acs_PA20_imm <- acs_PA20 %>% subset(BPL > 120)
acs_PA20_natz <- acs_PA20 %>% subset(CITIZEN == 2) 
acs_PA20_newNatz <- acs_PA20_natz %>% subset(YRNATUR >=2016)

###### ANALYSIS ###### 
#============================================================================
# PENN POPULATION COUNTS
#============================================================================
###### CALCULATE NUMBER OF IMMIGRANTS, TOTAL NATURALIZED IN PENN ######
sum(acs_PA20$PERWT) # penn population
sum(acs_PA20_imm$PERWT) # number of immigrants in penn
sum(acs_PA20_natz$PERWT) # number of naturalized citizens in penn
sum(acs_PA20_newNatz$PERWT)  # number of newly naturalized citizens in penn

####### CALCULATE NUMBER ELIGIBLE FOR CITIZENSHIP ####################
eligible_citizen <- acs_PA20_imm %>% subset(CITIZEN %in% c(3, 4) & YRSUSA1 > 5)
sum(eligible_citizen$PERWT)

####### PRE-TAX WAGE CONTRIBUTIONS ####################
# get rid of 99999/99998 values representing N/A or Missing incomes
incwage <- (acs_PA20_imm %>% subset(INCWAGE < 999998))
...
n_newnatz = 85083
natz_by_region <- as.data.frame(penn %>% group_by(Region.Name) %>% summarise(Freq = sum(PENNSYLVANIA)) %>%
                                  mutate(Percentage = Freq/n_newnatz))
View(natz_by_region)

# top ten country of origins
view(penn %>% arrange(desc(PENNSYLVANIA)))
...
#============================================================================
####### CALCULATE OCCUPATION PREVALENCE OF IMMIGRANTS IN PENN ###########
#============================================================================
# workforce populaiton in penn
n_penn = sum(acs_PA20$PERWT[acs_PA20$OCC != 0])
occ_general <- (
  acs_PA20 %>% group_by(category)  %>%
    summarise(n = sum(PERWT),
              Freq = sum(PERWT)/n_penn)
)

# immigrant workforce population
n_imm = sum(acs_PA20_imm$PERWT[acs_PA20_imm$OCC != 0])
occ_imm <- (
  acs_PA20_imm %>% group_by(category)  %>%
    summarise(n = sum(PERWT),
...
# =========================================
# export csvs
write.csv(penn, "Country_Region_Birth_OHIO.csv", row.names = F)
write.csv(county_region, "Natz_by_County_OHIO.csv", row.names = F)
write.csv(natz_by_region, "Natz_by_Region_OHIO.csv", row.names = F)
write.csv(acs_PA20, "out/acs_PA20.csv", row.names = F)
write.csv(acs_PA20_imm, "out/acs_PA20_imm.csv", row.names = F)
write.csv(acs_PA20_natz, "out/acs_PA20_natz.csv", row.names = F)
write.csv(acs_PA20_newNatz, "out/acs_PA20_newNatz.csv", row.names = F)
