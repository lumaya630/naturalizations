...
#============================================================================
####### CALCULATE OCCUPATION PREVALENCE OF IMMIGRANTS IN WISCONSIN ###########
#============================================================================
# workforce populaiton in wisconsin
n_wisconsin = sum(acs_WI20$PERWT[acs_WI20$OCC != 0])
occ_general <- (
  acs_WI20 %>% group_by(category)  %>%
    summarise(n = sum(PERWT),
              Freq = sum(PERWT)/n_wisconsin)
)

# immigrant workforce population
n_imm = sum(acs_WI20_imm$PERWT[acs_WI20_imm$OCC != 0])
occ_imm <- (
  acs_WI20_imm %>% group_by(category)  %>%
    summarise(n = sum(PERWT),
...
# =========================================
# export csvs
write.csv(wisconsin, "Country_Region_Birth_OHIO.csv", row.names = F)
write.csv(county_region, "Natz_by_County_OHIO.csv", row.names = F)
write.csv(natz_by_region, "Natz_by_Region_OHIO.csv", row.names = F)
write.csv(acs_WI20, "out/acs_WI20.csv", row.names = F)
write.csv(acs_WI20_imm, "out/acs_WI20_imm.csv", row.names = F)
write.csv(acs_WI20_natz, "out/acs_WI20_natz.csv", row.names = F)
write.csv(acs_WI20_newNatz, "out/acs_WI20_newNatz.csv", row.names = F)
