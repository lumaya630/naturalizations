...
# 1) Set up
# 2) Analysis
#     (a) North Carolina population counts
...
#============================================================================
####### CALCULATE OCCUPATION PREVALENCE OF IMMIGRANTS IN NORTH CAROLINA ###########
#============================================================================
# workforce populaiton in nc
n_nc = sum(acs_NC20$PERWT[acs_NC20$OCC != 0])
occ_general <- (
  acs_NC20 %>% group_by(category)  %>%
    summarise(n = sum(PERWT),
              Freq = sum(PERWT)/n_nc)
)

# immigrant workforce population
n_imm = sum(acs_NC20_imm$PERWT[acs_NC20_imm$OCC != 0])
occ_imm <- (
  acs_NC20_imm %>% group_by(category)  %>%
    summarise(n = sum(PERWT),
...
# =========================================
# export csvs
write.csv(nc, "Country_Region_Birth_OHIO.csv", row.names = F)
write.csv(county_region, "Natz_by_County_OHIO.csv", row.names = F)
write.csv(natz_by_region, "Natz_by_Region_OHIO.csv", row.names = F)
write.csv(acs_NC20, "out/acs_NC20.csv", row.names = F)
write.csv(acs_NC20_imm, "out/acs_NC20_imm.csv", row.names = F)
write.csv(acs_NC20_natz, "out/acs_NC20_natz.csv", row.names = F)
write.csv(acs_NC20_newNatz, "out/acs_NC20_newNatz.csv", row.names = F)
